...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define DISPLAY_PERIOD 50 //ms
#define ADC_Q 16 //exponencialna kumulace
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
	...
	if (channel == 0) {       //Channel 0 -> potentiometer conversion
		raw_pot = avg_pot >> ADC_Q;
		avg_pot -= raw_pot;
		avg_pot += HAL_ADC_GetValue(hadc);
	} else if (channel == 1) {  //Channel 1 -> Temperature
		raw_temp = HAL_ADC_GetValue(hadc);
	} else {					  //Channel 2 -> Buttons
...
int main(void) {
	...
	while (1) {
		...
		if (HAL_GetTick() >= lastDisplayTicks + DISPLAY_PERIOD) {
			...
			if (HAL_GetTick() >= lastBtnPress + SHOW_TIME) {
				state = SHOW_POT;
			}

			switch (state) {
				case SHOW_POT: {
					sct_value(raw_pot * 500.5 / 4095, (raw_pot * 8.5 / 4095));
					break;
				}
				case SHOW_TEMP: {
